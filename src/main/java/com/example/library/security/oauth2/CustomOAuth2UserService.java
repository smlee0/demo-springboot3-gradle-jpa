package com.example.library.security.oauth2;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		// 이메일로 가입된 회원인지 조회한다.
		Optional<AccountResponseDto> findAccount = accountService.findByEmail(email);
		...
		return new DefaultOAuth2User(
			Collections.singleton(new SimpleGrantedAuthority("ROLE_".concat(findAccount.get().getAccountRole().toString()))),
			memberAttribute, "email");
